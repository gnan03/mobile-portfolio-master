...
var resizePizzas = function(size) {
  ...
  var samplePizza=document.getElementsByClassName("randomPizzaContainer")[0];
  //Moved variables that don't need to be in the loop
  var oldWidth=samplePizza.offsetWidth;
  //Passing oldWidth as additional parameter for function, since it is calculated only once.
  var dx = determineDx(samplePizza, size, oldWidth);
  var newwidth = (oldWidth + dx) + 'px';
  var pizzaCollection =document.getElementsByClassName("randomPizzaContainer");
  // Iterates through pizza elements on the page and changes their widths
  function changePizzaSizes(size) {
    for (var i = 0; i < pizzaCollection.length; i++) {
      pizzaCollection[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  //Since phase has only 5 values, storeit in an array for future use
  for(var j=0;j<5;j++)
  {
    phase[j]=Math.sin(scrollTop+j);
  }
  for (var i = 0; i < items.length; i++) {
   translateVal=items[i].basicLeft + 100 * phase[i%5] + 'px';
   items[i].style.left = translateVal;
  }
